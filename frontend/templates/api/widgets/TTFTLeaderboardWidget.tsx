...
import { useTtftLeaderboardRows } from './leaderboardData';
import { WidgetTime } from './WidgetTime';

const IFRAME_CODE =
  '<iframe src="https://mazh.io/embed/leaderboard?metric=ttft&theme=dark" width="300" height="200" frameborder="0"></iframe>';

export function TTFTLeaderboardWidget() {
  const [size, setSize] = useState<WidgetSize>('s');
  const visibleRows = useTtftLeaderboardRows(size);

  const widthStyle = size === 's' ? { width: 300 } : size === 'm' ? { width: 640 } : undefined;
  const header = size === 's' ? 'TTFT Leaderboard' : 'TTFT Leaderboard – Time to First Token';

  return (
    <WidgetChrome
      id="widget-ttft-leaderboard"
      title="TTFT Leaderboard"
      description="Who's fastest? Ranked by Time to First Token."
      size={size}
